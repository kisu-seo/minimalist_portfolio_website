...
## 진행 중 / 현재 범위
- **현재 작업 범위: 모든 페이지 공통 적용** (2026-07-09 사용자 지시, 별도 안내 전까지 유지). 앞으로 요청하는 모든 스타일 및 구조 패치들은 공유 파일(`css/style.css`, `js/main.js`)에 작성하여 사이트 내 모든 웹페이지에 전역 반영되게 하거나, 각 HTML 파일에 공통 일괄 수정 적용.
